...
const packageKeyBlacklist = [
	'name', 'version', 'private', 'repository', 'author', 'license',
]

let validationErrors = ''
const addValidationError = (error) => validationErrors += `${validationErrors ? '\n' : ''}${error}`
...
const validateTemplate = (templateName, templatePath) => {
	...
	if (!packageJson.description)
		addErrorListItem('* package.json must contain a "description" property that describes your template')
...
console.log('INFO: Validating @intouchg/starter templates and generating metadata')
console.log('INFO: Read more about template requirements here: https://github.com/intouchg/starter#template-requirements')
...
if (validationErrors) {
	console.log(chalk.red(`ERROR: Failed to generate templates.json\n${validationErrors}\n`))
	process.exit(1)
}

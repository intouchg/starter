...
const validatePackageVersion = () => {
	...
	if (childProcess.status !== 0 || !semver.valid(latestVersion))
		return console.log(
			chalk.red(
				'ERROR: Failed to determine latest version of @intouchg/starter'
			)
		)
	...
	if (versionDiff === 1)
		console.log(
			chalk.red(
				`WARNING: Detected outdated version of @intouchg/starter, running ${pkg.version} but @latest is ${latestVersion}`
			)
		)
	...
	if (versionDiff === -1)
		console.log(
			chalk.yellow(
				`WARNING: Detected experimental version of @intouchg/starter, running ${pkg.version} but @latest is ${latestVersion}`
			)
		)
}
